...
| **`clashAnalysisFormat.ts`** | Normalizes run-analysis **`recommendations`** / **`watch_out_for`** strings into parsed objects for **`ClashInspector`** cards. Handles JSON objects and **Python `repr(dict)`-style** strings; values may use **double quotes** when the text contains apostrophes (API often does `str(dict)` per item), so parsing supports **`'key': 'val'`** and **`'key': "val"`**. |
