...
| UI          | React 19, Tailwind CSS v4 (`@import 'tailwindcss'` in `src/index.css`); **`react-markdown`** + **`remark-gfm`** render Markdown/GFM inside chat bubbles (**`ChatMarkdown`**). |
...
| **`ChatWindow`** | Chat panel body: SSE stream, **`GET`/`PUT /agent-config`**, history restore, **`AgentActivityLog`**. **Message bodies** (user **`text`** and assistant **`assistantBubbleText`** output) render through **`ChatMarkdown`** — lists, headings, fences, tables, links, tasks, etc. (**no raw HTML**; default **`ReactMarkdown`** URL handling only). Incomplete Markdown while streaming may briefly flicker until the fenced block/table closes. **`postChatStream`** **`onToolCall`** handler: when **`tool_name`** matches **`update_clash_recommendation`** (normalized aliases — see **`isUpdateClashRecommendationToolName`**), applies **`applyAgentRecommendationUpdate`** so the Recommendations panel updates during streaming without waiting for reload. Supports **chat attachments** via **`ChatAttachmentChips`** (draft pill row with remove `×`) and **`ChatAddContextMenu`** (`+` menu for clash / selected object / per-recommendation items). Recommendation attachments support **`attach`** (one-shot context) vs **`modify`** (sticky chip + directive for the agent); after send, **`clearOneShotAttachments`** drops clash/selected/`attach` chips but keeps **`modify`** chips until removed. **`composerFocusEpoch`** from **`FloatingChatContext`** refocuses the composer when launching Modify from the inspector. On send, draft attachments are snapshotted into **`POST /chat`**, cleared from the composer (per **`clearOneShotAttachments`**), and **`ChatAttachmentSummary`** (`kind` + `label`) is stored on the **user message** so **`ChatMessageAttachmentChips`** shows the same context on the bubble (**stacked above** the markdown user copy). **`GET /chat/messages`** returns those summaries for user rows so a **browser refresh** (same API process + session conversation id) rehydrates chips. Header is drag-handle enabled (except interactive controls). **Agent settings** UI is rendered via **`createPortal`** with viewport-aware **`position: fixed`** placement so coordinates stay correct above backdrop-blur / transformed ancestors. |
| **`ChatMarkdown`** | **`components/layout/ChatMarkdown.tsx`**. Wraps **`react-markdown`** with **`remarkPlugins: [remarkGfm]`** and Tailwind-styled element overrides; the **`variant`** prop (**`user`** \| **`assistant`**) adjusts inline-code tint to match each bubble. Used from **`ChatWindow`** only. |
...
| **`AppProvider.tsx`** | Implements state: clash upload/session hydration, Speckle row CRUD with stable **`id`**s, derived **`speckleUrls`** string array for viewers/guards. Selecting a clash turns off severity-highlight mode; enabling severity highlight clears selected clash. **Object notes** persist under **`balrog-object-metadata`** in **`localStorage`**; **`clearSession`** wipes them and removes that key. Also stores shared **`selectedObjectData`** and **`speckleViewer`** for cross-feature consumers (chat attachment menu + overlay). **`clearSession`** does **not** clear **`ClashAnalysisContext`** persistence (**`balrog-clash-analysis-v1`**); use **`clearAllAnalysis`** (e.g. onboarding “new report” flows) to wipe stored run-analysis snapshots. |
| **`useApp.ts`** | Hook to consume context (throws if outside provider). |
| **`FloatingChatContext.tsx`** | **`FloatingChatProvider`** + **`useFloatingChat()`**: **`isChatOpen`**, **`setChatOpen`**, **`toggleChat`**. Persists **`balrog-floating-chat-open`** in **`localStorage`**. **`AppLayout`** nests this provider around **`ClashAnalysisProvider`** so **`FloatingChat`**, **`FloatingNavbar`**, and **`InspectorToolbar`** share one chat visibility state. **`composerFocusEpoch`** + **`requestComposerFocus()`**: incremented when something should focus the chat textarea after opening chat (e.g. **Modify** on a recommendation). |
| **`ChatAttachmentsContext.tsx`** | One-turn attachment draft state for chat: `attachments[]`, `addAttachment` (dedupe/replace by stable `id`), `removeAttachment`, `clearAttachments`, **`clearOneShotAttachments`** (removes clash, selected-object, and **`attach`**-mode recommendation chips; keeps **`modify`**-mode recommendation chips sticky), **`removeRecommendationAttachmentsForClash`** (e.g. after re-run analysis), `hasAttachment`. Recommendation rows carry **`recommendationIndex`** and **`mode`** (**`attach`** \| **`modify`**). |
| **`ClashAnalysisContext.tsx`** | Hoisted run-analysis output keyed by `clashId` (`recommendations`, watch list, scratchpad metadata, notes, etc.) for **`ClashInspector`**, chat recommendation attachments, and agent-driven edits. **Persisted in `localStorage`** under **`balrog-clash-analysis-v1`** (survives browser refresh; independent of API process restarts). **`clearAllAnalysis`** clears React state and removes that key; **`clearAnalysisForClash`** drops one id. Updates merge via **`setAnalysisForClash`** (initial load + **`applyAgentRecommendationUpdate`** on SSE **`tool_call`**). |
...
5. Chat attachment sources (Context panel, Recommendations list, Selected object overlay, chat `+` menu) all write into **`ChatAttachmentsContext`**. On send, `ChatWindow` snapshots attachments into the **`POST /chat`** payload, runs **`clearOneShotAttachments`** (keeps **`modify`** recommendation chips), attaches display **`ChatAttachmentSummary`** metadata to the optimistic user message, and streams the assistant reply. Incoming SSE **`tool_call`** events for **`update_clash_recommendation`** update **`ClashAnalysisContext`** in place via **`applyAgentRecommendationUpdate`**. History reload maps **`attachments`** from **`GET /chat/messages`** onto user **`ChatMessage`** rows (see **`chatHistory.ts`**).
...
| **`postChatStream.ts`** | **`postChatStream(message, conversationId, handlers, options?)`** — **`POST /chat`** with **`Accept: text/event-stream`**, parses SSE (`metadata`, `token`, `thought_delta`, `agent_thought`, `tool_call`, `tool_result`, `done`, `error`). Optional **`handlers.onToolCall`** (used for **`update_clash_recommendation`** UI sync). Supports `options.signal` + `options.attachments`. Includes request-size guard via **`CHAT_MAX_BODY_BYTES`** and **`ChatPayloadTooLargeError`** before fetch. |
| **`chatAttachments.ts`** | Frontend wire schema + converters for chat attachments (`toChatAttachmentWire`, `toChatAttachmentsWire`), including **`resolveRecommendation`** for embedding current recommendation JSON from **`ClashAnalysisContext`**. Client-side `object_data` trimming (`HIDDEN_OBJECT_DATA_KEYS`). Wire includes **`recommendation_index`** / **`mode`** for recommendation attachments. |
| **`buildChatAttachments.ts`** | Builders for `clash`, `selected_object`, and `recommendation` attachment objects with stable IDs and user-facing labels; recommendation builders take **`recommendationIndex`** and **`mode`**. |
...
| **`applyAgentRecommendationUpdate.ts`** | Validates **`tool_kwargs`** from **`update_clash_recommendation`** (and normalized name aliases), merges **`recommendation`** into **`ClashAnalysisContext`** via **`setAnalysisForClash`**. **`isUpdateClashRecommendationToolName`** guards **`ChatWindow`** SSE handling. |
