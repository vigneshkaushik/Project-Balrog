...
| **`ChatWindow`** | Chat panel body: SSE stream, **`GET`/`PUT /agent-config`**, history restore, **`AgentActivityLog`**. Supports **chat attachments** via **`ChatAttachmentChips`** (pill row with remove `×`) and **`ChatAddContextMenu`** (`+` menu for clash / selected object / per-recommendation items). Attachments are captured into the outgoing payload and cleared **immediately on user send**. Header is drag-handle enabled (except interactive controls). **Agent settings** UI is rendered via **`createPortal`** with viewport-aware **`position: fixed`** placement so coordinates stay correct above backdrop-blur / transformed ancestors. |
| **`ChatAttachmentChips`** | Horizontal chip row above the chat textarea; one chip per attached item with icon, truncated label, and remove action. |
...
5. Chat attachment sources (Context panel, Recommendations list, Selected object overlay, chat `+` menu) all write into **`ChatAttachmentsContext`**. On send, `ChatWindow` snapshots attachments into payload, clears chips immediately, and streams response as usual.
